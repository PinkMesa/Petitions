...
import {signIn, socialSignIn} from "../../redux/actions/auth";
...
const Signin = () => {
  ...
  const socialResponse = (providerId, response) => {
    switch(providerId) {
      case 'facebook': {
        ...
        setIsLoading(true);
        dispatch(socialSignIn(firstName, lastName,
          response.email, response.email, response.accessToken, response.id,'facebook'))
      ...
      case 'google': {
        ...
        setIsLoading(true);
        dispatch(socialSignIn(profileObj.givenName, profileObj.familyName,
          profileObj.email, profileObj.email, response.accessToken, response.googleId, 'google'))
          .finally(() => setIsLoading(false));
        break;
      }
    }
